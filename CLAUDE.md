...
- [x] 1단계 CI/CD 환경 구축 (GitHub Actions + Fastlane, 완전 무료, 환경 분리, 최소 스켈레톤)
...
빌드/배포는 **무료 도구(GitHub Actions + Fastlane)** 가 담당한다(EAS 클라우드 미사용).
로컬 네이티브 빌드가 필요하면 `npm run prebuild` 후 `gradlew`(Android) / Xcode·Fastlane(iOS).
릴리즈/스토어 제출은 CI(`release.yml`)가 수행하므로 로컬 수동 실행은 지양한다.
...
`APP_ENV` = `development` / `staging` / `production`. 설정 위치: `app.config.ts`(앱 메타), `src/config/env.ts`(런타임), 스토어 제출 식별자는 `fastlane/Appfile`. 자세한 내용은 `README.md`.
