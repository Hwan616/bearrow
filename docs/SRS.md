...
| IF-002 | 시스템은 Apple 캘린더(EventKit/CalDAV)와 연동될 수 있어야 한다. | C | Active | 1.0.0 |
...
# 부록 A. 요구사항 추적 (Traceability)

요구사항 ID 는 `docs/BACKLOG.md` 의 작업 항목과 매핑된다(예: `FR-CAL-002` ↔ 백로그 `1.2 월 뷰`). 구현·테스트 시 커밋 메시지/PR 에 해당 요구사항 ID 를 함께 표기해 추적성을 확보한다.

# 부록 B. 미해결 사항 (TBD)

| ID | 항목 | 비고 |
| --- | --- | --- |
| TBD-001 | Apple/CalDAV 동기화 상세 범위 | 후속 버전에서 확정 |
