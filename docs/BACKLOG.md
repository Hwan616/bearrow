...
작업 형식: `ID · 제목` → 완료 조건 / 주요 파일 / 프롬프트 예시.
...
- [x] **0.2 환경 분리** — `app.config.ts`, `src/config/env.ts`
...
- [ ] **5.3 성능·접근성 점검** — 리스트 가상화, 대비/폰트 스케일.
- [ ] **5.4 (선택) self-hosted OTA** — `expo-updates` 자체 호스팅 업데이트 서버 구성(무료). NFR-MNT-001 충족.
...
완료 조건·주요 파일을 기재한 뒤 사용자에게 우선순위를 확인한다.
