...
- [x] **7.22 YearView 미니 달력 일요일 색상 통일** — 일요일 날짜를 평일과 동일 색으로. (커밋 97afd2f)
- [x] **7.23 '오늘' 버튼 뷰 계층 동작 구현·정밀화** — Year/Month/Day별 가시성 기반 이동, `isMonthVisible`·`isWeekVisible` 핸들. (커밋 9538891, b7f2a3f)
- [x] **7.24 MonthView 오류 수정·레이아웃 정리** — 초기 스크롤 정렬, 이벤트 바, 서브타이틀, 셀 상단 정렬. (커밋 706c5fe, 33920a0, f779774)
- [x] **7.25 MonthView 이벤트 바 상단 정렬 + 초과 indicator** — `n-1`개 표시 + `+x개`(날짜 단위). (커밋 9fc6528, 8de6f4b)
- [x] **7.26 MonthView 6단계 이산 줌 + 핀치 확대/축소** — dots/thin_bars/bars, 연속 핀치→가장 가까운 레벨 스냅. (커밋 76711d5, 8f271c4)
- [x] **7.27 일정 편집 UX 개선** — 바 탭 시 EventForm 직접 열기, 섹션 재구성, 라이트 모드 회색 섹션. (커밋 8b25ebf)
- [x] **7.28 반복 일정 기능 강화 + CRUD 버그 수정** — 2주마다·매년·종료조건·범위 편집/삭제, makeDefaultValues 복원 버그. (커밋 2b71135)
- [x] **7.29 DayView 날짜 레이블·카테고리 색상·겹침 컬럼** — YYYY.MM.DD.(W), 카테고리 색+불투명도, `layoutTimedEvents`. (커밋 65b2eb7)
- [x] **7.30 스타일 8종 개선** — 박스 레이어링·카테고리 +·꺾쇠·YearView 정렬·하단바 스크림·형광 팔레트·DayView. (커밋 af44641)
- [x] **7.31 네비 아이콘 수정** — 꺾쇠 세로축 반전, 설정 6톱니·빈 원 링. (커밋 9191864)
- [x] **7.32 다크 모드 회색 밝게 + 시각 피커 다크 대응** — 배경/surface/disabled 조정, DateTimePicker themeVariant. (커밋 d8b4eb1)
- [x] **7.33 성능·UX** — 월별 캐시, FlatList 튜닝, 기본 시각 5분, 알림·반복 영역 분리. (커밋 30600d9)
- [x] **7.34 뷰 전환 확대/축소 모션** — 계층 깊이 기반 스케일+페이드(reanimated). (커밋 74756f5)
- [x] **7.35 핀치 확대/축소 재설계** — 셀 높이 실시간 변경·상세도 단계 전환·터치 앵커 고정·줌 단계 영속. (커밋 f15990e, 0e02223, 42f64e9)
